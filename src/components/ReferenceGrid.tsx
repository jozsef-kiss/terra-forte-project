    ...
    <>
      <div className="bg-white py-24 sm:py-32 select-none">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          {/* MINIMALISTA RÁCS 
             - Kivettem a "layout" propot (ez okozta a nyúlást).
             - Kivettem az AnimatePresence-t innen (ez okozta, hogy megvárta amíg eltűnik a régi).
          */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredItems.map((item) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  // Csak egy finom áttűnés (opacity) a megjelenéskor
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
      ...
      {/* LIGHTBOX / MODAL (Itt maradt az animáció, mert itt jól néz ki) */}
      <AnimatePresence>
        {selectedImage && (
          <motion.div
            initial={{ opacity: 0 }}
